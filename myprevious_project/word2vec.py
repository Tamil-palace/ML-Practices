...
fileName="334-Catalog-20180604_p1.csv"
...
for index,temp1 in enumerate(nr_corpus):
    ...
    for i in range(num_best):
        ...
        with open("similarity.txt","a") as fh:
            fh.write(str(nr_corpus[index][1])+"\t"+str(nr_corpus[index][0].split("¶")[0])+"\t"+str(nr_corpus[index][0].split("¶")[1])+"\t"+str(nr_corpus[index][0].split("¶")[2])+"\t"+str(nr_corpus[index][0].split("¶")[3])+"\t"+str(src_corpus_original[sims[i][0]][1])+"\t"+str(src_corpus_original[sims[i][0]][0].split("¶")[0])+"\t"+str(src_corpus_original[sims[i][0]][0].split("¶")[1])+"\t"+str(src_corpus_original[sims[i][0]][0].split("¶")[2])+"\t"+str(src_corpus_original[sims[i][0]][0].split("¶")[3])+str(sims[i][1])+"\n")
