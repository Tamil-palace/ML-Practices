...
Exclude_Words=eval(config.get("Title-Exclusion-Words","Exclude-Words"))
# print(Exclude_Words[0])
...
if __name__ == "__main__":
    ...
    r.set(str(cNumber) + "_" + str(date) + "_2_total", len(inputData))
    ...
    for i, dataVal in enumerate(inputData):
        ...
        if processedTrack == 1:
            ...
            print(validSims[query_doc_tf_idf])
            input()
                    ...
                    if float(fuzzyScore) >= 85:
                        if 'n-' in str(SourceList_updated[index][trackItemIndex]).lower() or 'n -' in str(SourceList_updated[index][trackItemIndex]).lower():
                            ...
                            if float(fuzzyScore_2nd) >=85 :
                                if "n-" in str(SourceList_updated[index_2nd][trackItemIndex]).lower() or 'n -' in str(SourceList_updated[index_2nd][trackItemIndex]).lower():
                        ...
                        if manufacture_fuzzyScore >= 95 and brand_fuzzyScore >= 95:
                            print(""+str(manufacture_fuzzyScore)+" - "+str(brand_fuzzyScore)+" - "+str(manufacture)+" - "+str(SourceList_updated[index][brandIndex])+" - "+str(brand))
                            with open("issue.txt","a") as fh:
                                fh.write(""+str(manufacture_fuzzyScore)+" - "+str(brand_fuzzyScore)+" - "+str(manufacture)+" - "+str(SourceList_updated[index][brandIndex])+" - "+str(brand))
