...
descriptionIndex = inputHeader.index("description")
featuresIndex = inputHeader.index("features")
...
for index,data in enumerate(reader):
    ...
    if not re.findall(r"Needs", str(reader[index][inputHeader.index("Track Item")]), re.I):
        ...
        data=str(reader[index][titleIndex])+"¶"+str(reader[index][descriptionIndex])+"¶"+str(reader[index][featuresIndex])
        id=str(reader[index][idIndex])
        temp.append(id)
        temp.append(data)
        all.append(temp)
        continue
    data_1 = str(reader[index][titleIndex]) + "¶" + str(reader[index][descriptionIndex]) + "¶" + str(reader[index][featuresIndex])
        ...
        try:
            if (j < len(test_list)):
                ...
                try:
                    ...
                    writ = str(all[i][0])+"\t"+str(all[i][1]).split("¶")[0]+"\t"+str(all[i][1]).split("¶")[1]+"\t"+str(all[i][1]).split("¶")[2]+"\t"+str(test_list[j][0])+"\t"+str(test_list[i][1]).split("¶")[0]+"\t"+str(test_list[i][1]).split("¶")[1]+"\t"+str(test_list[i][1]).split("¶")[2]+"\t"+ str(k1)+ "\n"
                    if (k1 <= 3.5):
                        print(k1)
                    with open('Output_25_09_desc_feature.txt', 'ab') as f:
                        f.write(str(writ))
                    # else:
